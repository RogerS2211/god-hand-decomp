...
A pre-bootstrapped Ghidra project + MCP bridge so an MCP client can query
Ghidra (decompile, xrefs, structs) over MCP. Installed under ~ (WSL).
...
- MCP client config → .mcp.json (project root; gitignored).
...
- Reload your MCP client so it reads .mcp.json; approve the `ghidra` server.
