extern int D_00747A34;
extern char D_00754C10[];

__attribute__((section(".text.func_003173B0")))
int func_003173B0(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x8E0, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x8E0);
    return func_0017AAD8(s0);
}

__attribute__((section(".text.func_00317420")))
int func_00317420(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x1DB0, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x1DB0);
    return func_0017B870(s0);
}

__attribute__((section(".text.Setup_0x780_17C4A8_317490")))
int Setup_0x780_17C4A8_317490(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x780, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x780);
    return func_0017C4A8(s0);
}

__attribute__((section(".text.func_00317500")))
int func_00317500(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x8A0, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x8A0);
    return func_0017C620(s0);
}

__attribute__((section(".text.func_00317570")))
int func_00317570(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x6B0, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x6B0);
    return func_0017D3D8(s0);
}

__attribute__((section(".text.func_003175E0")))
int func_003175E0(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x680, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x680);
    return func_0017D5D0(s0);
}

__attribute__((section(".text.func_00317650")))
int func_00317650(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x780, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x780);
    return func_0017E0F8(s0);
}

__attribute__((section(".text.func_003176C0")))
int func_003176C0(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x6B0, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x6B0);
    return func_0017E828(s0);
}

__attribute__((section(".text.func_00317730")))
int func_00317730(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x780, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x780);
    return func_0017EA98(s0);
}

__attribute__((section(".text.func_003177A0")))
int func_003177A0(void){
    void *s0;
    if (D_00747A34 & 0x10000) return 0;
    s0 = func_0030EE08(0x600, 0x10, D_00754C10);
    if (s0 == 0) return 0;
    func_003A52F0(s0, 0, 0x600);
    return func_0017EC08(s0);
}
