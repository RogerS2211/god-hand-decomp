#include "godhand/vu0.h"

/* func_0018A598 - object constructor: base init, set vtable at 0x214, then loop zeroing
 * a 5-quadword sub-block per slot and tagging it (Obj0000_Set_Byte_54). Loop
 * uses the while(i != -1) form to match retail bne-to-(-1). sn-2.95.3-136. */

extern void InitFields_1B6E90(void *);
extern void Obj0000_Set_Byte_54(void *, int);
extern int D_00423700;
__attribute__((section(".text.func_0018A598")))
void *func_0018A598(void *a0){
  char *s0; int i;
  InitFields_1B6E90(a0);
  s0 = (char*)a0 + 0x610;
  *(int**)((char*)a0+0x214) = &D_00423700;
  i = 7;
  do {
    VU0_SQC2_VF0(s0, 0x0);
    VU0_SQC2_VF0(s0, 0x10);
    VU0_SQC2_VF0(s0, 0x20);
    VU0_SQC2_VF0(s0, 0x30);
    VU0_SQC2_VF0(s0, 0x40);
    Obj0000_Set_Byte_54(s0, 0);
    s0 += 0x60;
    i--;
  } while(i != -1);
  return a0;
}
